...
(function (Drupal) {
  ...
  Drupal.behaviors.dcloudConfigDownload = {
    attach: function (context, settings) {
      ...
      once('dcloud-download', '.dcloud-config-download-button', context).forEach(function (button) {
        button.addEventListener('click', function () {
          const targetId = this.getAttribute('data-target');
  ...
  function downloadFile(element, filename, button) {
    ...
    try {
      ...
      button.textContent = "❌ Failed";
      button.classList.add('dcloud-config-download-button--error');
      setTimeout(function () {
        button.textContent = "💾 Download";
        button.classList.remove('dcloud-config-download-button--error');
      }, 2000);
    }
  }
  ...
  function showDownloadSuccess(button) {
    ...
    button.classList.add('dcloud-config-download-button--success');
    setTimeout(function () {
      button.textContent = originalText;
      button.classList.remove('dcloud-config-download-button--success');
    }, 2000);
  }
